...
def preview_hog_image():
    """
    随机选取一幅含有汽车的图片，显示其HOG特征；顺便也显示一张不含汽车的图片及其HOG特征
    :return: 无
    """
    ...
    car_features, car_hog_img = single_img_features(
        car_img,
    ...
    notcar_features, notcar_hog_img = single_img_features(
        notcar_img,
        color_space=color_space,
        spatial_size=spatial_size,
        hist_bins=hist_bins,
...
def search_classify():
    """
    读取6张不同的输入图片，滑动窗口，提取窗口区域的HOG特征，调用模型，判断是否包含汽车
    将所有包含汽车的区域全部显示出来
    :return: 无
    """
...
def hog_subsampling_detect():
    """
    读取6张图不同的输入图片，提取整个图片的HOG特征，滑动窗口，调用模型，判断窗口区域是否包含汽车
    将所有包含汽车的区域全部显示出来。跟上面的search_classify()函数差不多
    :return: 无
    """
...
def draw_heatmap_bbox():
    """
    在hog_subsampling_detect()的操作基础上，计算热度，合并重复识别的区域为单一区域，同时将误报区域删除
    最后显示的图像，左半部分为消除了误报区域的单一区域，右半区域为热度图
    :return: 无
    """
    # 1) Set the model file to load, search position and image sampling scale;
